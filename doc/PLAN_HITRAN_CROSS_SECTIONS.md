...
5. **Nouvelles fonctions de section efficace** :
   ...
   - **Troncature ailes lointaines** (hitran.js v1.3.1, standard LBL) : chaque raie ne contribue que si `|δν| ≤ VOIGT_N_WIDTHS × (γ_L + γ_D)` avec `VOIGT_N_WIDTHS = 100` (cf. `crossSectionFromLines`). Au-delà, l'approximation Faddeeva (Humlíček) décroche sous l'eps machine (~1e−16 sur le pic) → `Re(w)` peut sortir légèrement négatif, ce qui rend σ négatif et, en aval, `κ = σ·n < 0` → `τ < 0` → `1−exp(−τ) = −∞` → `integral = −∞` → capacités 🌈 écrasées à 0 par le clamp `[0,1]`. La troncature est physique : les ailes au-delà de ~100 largeurs ne contribuent pas à la section efficace observable (en pratique, continuum MT_CKD prend le relais pour H₂O), et le résidu numérique sous seuil n'a pas de sens physique. La fenêtre globale de préfiltrage `HALF_WINDOW_CM = 5 cm⁻¹` reste en place pour l'indexation binaire des lignes.
   - **Limite de la troncature seule** : si `100 × (γ_L + γ_D) ≥ HALF_WINDOW_CM`, la fenêtre de 5 cm⁻¹ ne contient aucun point au-delà du seuil — la troncature ne retire aucune ligne ; un σ négatif peut persister (bench epoch : CH₄ λ≈2.99 µm).
   - **Contrainte pdf** (hitran.js v1.3.2) : dans `voigtNormalized`, `Math.max(0, Re(w))` avant division par `γ_D√π` — le Voigt est une pdf (≥0) ; on restaure la définition mathématique contre l'artefact Faddeeva dans les ailes.
