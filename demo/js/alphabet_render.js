// File: API_BILAN/demo/alphabet_render.js - Rendu de l'alphabet : lexique, pictos, logos
...
//       définitions (CHARS, CHARS_DESC) ne sont pas ici : elles vivent dans API_BILAN/data/alphabet.js,
//       que je lis par window — je dois donc être chargé APRÈS lui. Je vis dans demo/ et non dans CO2 :
//       dessiner l'alphabet de l'API fait partie de ce qui illustre l'API, et l'application m'y lit.
// Version 1.1.0
// Date: [September 19, 2026]
// logs :
//   - v1.1.0: déplacé de CO2/static/compute/ vers API_BILAN/demo/. resolveImagePath connaît les pages
//     de demo/, qui ont leur propre fonts/pics/ (le dépôt API doit se tester sans CO2).
...
function resolveImagePath(path) {
    ...
    // Pages de démonstration (API_BILAN/demo/) : elles ont leur propre fonts/pics/, pour que le dépôt
    // API soit testable sans CO2. Le chemin est déjà bon tel quel.
    if (/\/demo\//.test(base)) return path;
    const inCompute = base.includes('/static/compute/') || base.includes('static\\compute\\');
    if (inCompute) {
